...
<#macro title>
    ...
    <script>
        function showResult() {
                ...
                if (this.readyState===4 && this.status===200) {
                    ...
                    if (response.length !== 0) {
                        for (let i = 0; i < response.length; i++) {
                            html += "<a href='/detailArticle/" + response[i]['id'] + "/1" + "'/>"
                            ...
                            html += "<br><div><small class='text-muted' style='font-size:20px'><a href='/deleteArticle/" + response[i]['id'] + "'>Удалить</a></small></div>"
                            html += "</div>" + "</a>"
                        }
                    } else {
            ...
            xmlhttp.open("GET","/allMyFindArticles?title=" + document.getElementById('title').value, true);
            xmlhttp.send();
        }
    </script>
</#macro>
...
<#macro content>
    ...
    <div id="result">
        <#if myArticles?has_content>
            <#list myArticles as article>
                <a href="/detailArticle/${article.id}/1">
                    <div class="alert alert-dark" role="alert">
                        ...
                        <div>
                            <small class="text-muted" style="font-size:20px">
                                <a href="/deleteArticle/${article.id}">Удалить</a>
                            </small>
                        </div>
                    </div>
                </a>
